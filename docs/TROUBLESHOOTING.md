...
2. **Check CORS:**
   - CORS is restricted to an explicit origin allowlist. `run_backend.py` sets
     `ALLOWED_ORIGINS` to the loopback origins for the port it picked; the API
     reads that variable at startup and logs the resulting list.
   - If the UI cannot reach the API, check that log line first — a dynamic port
     that is not in the allowlist is the usual cause.
   - It is deliberately **not** `allow_origins=["*"]`. With
     `allow_credentials=True` that echoes back any caller's Origin, which would
     let any website the user is browsing drive this API.
